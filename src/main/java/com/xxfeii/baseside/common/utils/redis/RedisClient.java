...
public class RedisClient {
	...
	public RedisClient() {
		...
		//jedis1 = jedisSentinelPool.getResource();
	}
	...
	private JedisPoolConfig getJedisPoolConfig() {
		...
		config.setMaxIdle(RedisConstant.REDIS_POOL_MAXIDLE);
		config.setMinIdle(RedisConstant.REDIS_POOL_MINIDLE);
		config.setMaxWaitMillis(RedisConstant.REDIS_POOL_MAXWAIT);
		config.setTestOnBorrow(RedisConstant.REDIS_POOL_TESTONBORROW);
		config.setTestOnReturn(RedisConstant.REDIS_POOL_TESTONRETURN);
		return config;
	}
	...
	public JedisPool initialPool() {
		...
		String[] redis_host = RedisConstant.REDIS_HOSTS.split(RedisConstant.SEPARATION_SIGN1);
		String[] host = redis_host[0].split(RedisConstant.SEPARATION_SIGN2);
		jedisPool = new JedisPool(config, host[0], Integer.valueOf(host[1]),RedisConstant.REDIS_TIMEOUT,RedisConstant.REDIS_PASSWORD);
		return jedisPool;
	}
	 ...
	 * 
	 */
	public ShardedJedisPool initialShardedPool() {
		// 池基本配置

		JedisPoolConfig config = getJedisPoolConfig();
		// slave链接
		List<JedisShardInfo> shards = new ArrayList<JedisShardInfo>();
		String[] redis_hosts = RedisConstant.REDIS_HOSTS.split(RedisConstant.SEPARATION_SIGN1);
		int size = redis_hosts.length;
		for(int i=0;i<size;i++){
			String[] host = redis_hosts[i].split(RedisConstant.SEPARATION_SIGN2);
			JedisShardInfo jedisShardInfo = new JedisShardInfo(host[0], Integer.valueOf(host[1]));
			jedisShardInfo.setPassword(RedisConstant.REDIS_PASSWORD);
	...
	public JedisSentinelPool initJedisSentinelPool() {
		...
		JedisPoolConfig config = getJedisPoolConfig();
		// slave链接
		String jpassword = RedisConstant.REDIS_PASSWORD;
		int timeout = RedisConstant.REDIS_TIMEOUT;
		String master = RedisConstant.REDIS_MASTER;
		String sentinels = RedisConstant.REDIS_SENTINELS;
		String[] sentinelArray = sentinels.split(RedisConstant.SEPARATION_SIGN1);
		Set<String> sentinelSet = new HashSet<String>();
		CollectionUtils.addAll(sentinelSet, sentinelArray);
		jedisSentinelPool = new JedisSentinelPool(master, sentinelSet, config, timeout, jpassword);
	 ...
	 * @param redis
	 */
	public static void emancipate(Pool<Jedis> pool, Jedis redis) {
		if (redis != null) {
	...
	public void aaa() {
		...
		System.out.println("判断key001是否存在：" + jedis1.get("key001"));
	}
